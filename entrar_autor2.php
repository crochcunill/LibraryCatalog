<?php
	...
	if (checklenght($HTTP_POST_VARS['autor_nom'])=="false" || $autor_nom_res[0]=="false" ||$autor_cognoms_res[0]=="false" || $autor_biografia_res[0]=="false") {
		...
		include("common_variables.php");		$query_string="INSERT INTO autors (autor_nom,autor_cognoms,autor_idioma_ID,autor_biografia) VALUES ('" . $HTTP_POST_VARS['autor_nom']. "','" . $HTTP_POST_VARS['autor_cognoms'] . "','" .$HTTP_POST_VARS['autor_idioma_ID']. "','" .$HTTP_POST_VARS['autor_biografia']. "')";	
		...
		if (!empty($MYSQL_ERROR)){		
				...
				header("Location: ./tots_autors_nom.php?validacio=".$validacio."&flag=2&autor_nom=".$HTTP_POST_VARS['autor_nom']. "&autor_cognoms=" . $HTTP_POST_VARS['autor_cognoms']."&pagina_actual=1"); /* Redirect browser */
				exit;
				}	
