<?php
	...
	if ($_GET['flag']==1){echo "<P>L'autor/a <B>".  changeslashbar($_GET['autor_nom'])." ". changeslashbar($_GET['autor_cognoms']). "</b> ha estat esborrat";}
	if ($_GET['flag']==2){echo "<P>L'autor/a <B>".  changeslashbar($_GET['autor_nom'])." ". changeslashbar($_GET['autor_cognoms']). "</b> ha estat incorporat a la base de dades";}
	if ($_GET['flag']==3){echo "<P>Les dades de l'autor/a <B>".  changeslashbar($_GET['autor_nom'])." ". changeslashbar($_GET['autor_cognoms']). "</b> s'han editat correctament";}		
	include("common_variables.php");	
	$my_query="SELECT autor_ID,autor_nom,autor_cognoms,idioma_nom,autor_biografia from autors,idiomes where autor_idioma_ID=idioma_ID ORDER BY autor_cognoms";							
		...
		for ($i=1;$i<=$numero_pagines;$i++){
			if ($i==1){
				if ($i==$pagina_actual){
					...
					echo "<A HREF=./tots_autors_cognoms.php?validacio=".$validacio."&pagina_actual=".$i.">".$i."</A>";
					}
				}
			else{
				if ($i==$pagina_actual){
