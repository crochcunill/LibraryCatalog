<?php
	...
	if (checklenght($paraules_clau)=="false" ) {
		$paraules_clau=$_GET['paraules_clau'];
		$pagina_actual=$_GET['pagina_actual'];	
		}	

	if (checkvalues($paraules_clau)=="false") {
		echo "<P>Hi han caracters no valids en el que heu entrat <I><?php echo $paraules_clau ?></I>. Intenteu de nou";				$PerformQuery=0;							}
	elseif (checklenght($paraules_clau)=="false" ) {		echo "<P>Heu d'entrar alguna cosa. Intenteu de nou";				$PerformQuery=0;							}				else{		$begin_query="Select titol_ID,titol_nom,autor_nom,autor_cognoms,autor_ID,idioma_nom from titols,autors,idiomes where (";
		$end_query=") AND (titol_autor_ID=autor_ID) AND (titol_idioma_ID=idioma_ID)";							$title_elements=explode(" ",$paraules_clau);
		...
		for($counter=0;$counter<$tamany_array;$counter++){			
				...
				if ($counter<$tamany_array-1){			
					$mid_query=$mid_query. " titol_sinopsis LIKE '%".$element."%' OR titol_nom LIKE '%".$element."%' OR ";
					}
				else{
					$mid_query=$mid_query. "titol_sinopsis LIKE '%".$element."%' OR titol_nom LIKE '%".$element."%'";
					}	
			}
...
?>		
	<p>Hi han <B><?php echo $num_entrades?></B> títol/s a la base de dades que 
	 contenen <B>"<?php echo $paraules_clau ?>"</B> en el títol o a la sinopsis. 
	 Els trobareu ordenats pel seu <B>titol</B> i distribuïts en <B><?php echo $numero_pagines?></B> pàgines. La 
	...
	if ($pagina_actual-$paginacio>=1  and $pagina_actual+$paginacio<$numero_pagines){
		echo  "<A HREF=./paraules_clau_p.php?paraules_clau=".urlencode($paraules_clau)."&pagina_actual=1>"."<<"."</A>"." | ";
		for ($i=$pagina_actual-$paginacio;$i<=$pagina_actual+$paginacio;$i++){
			if ($i==$pagina_actual){
				echo $i;
			}
			elseif($i<$pagina_actual){
					echo  "<A HREF=./paraules_clau_p.php?paraules_clau=".urlencode($paraules_clau)."&pagina_actual=".$i.">".$i."</A>"." | ";
			}
			else {echo " | ". "<A HREF=./paraules_clau_p.php?paraules_clau=".urlencode($paraules_clau)."&pagina_actual=".$i.">".$i."</A>";}	
		}
		echo  " | "."<A HREF=./paraules_clau_p.php?paraules_clau=".urlencode($paraules_clau)."&pagina_actual=$numero_pagines>".">>"."</A>";	
	}
