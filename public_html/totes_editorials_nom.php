...
if ($validacio!=session_id()){
	...
	if ($_GET['flag']==1){echo "<P>La col.lecció <B>". $_GET['editorial_colleccio']." </B> de l'editorial <B>".  $_GET['editorial_nom']. "</b> ha estat esborrada";}
	
	if ($_GET['flag']==2){echo "<P>La col.lecció <B>". $_GET['editorial_colleccio']." </B> de l'editorial <B>".  $_GET['editorial_nom']. "</b> ha estat incorporat a la base de dades";}

	if ($_GET['flag']==3){echo "<P>Les dades de l'editorials/a <B>".  $_GET['editorial_nom']." ". $_GET['editorial_colleccio']. "</b> s'han editat correctament";}
...
<?php
	...
	//			$database_ID=mysql_select_db("Casal_Biblioteca",$link_ID);
				...
				$resultats=mysqli_query($link_ID,"SELECT * from editorials ORDER BY editorial_nom");
